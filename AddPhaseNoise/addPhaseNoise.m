...
if ~any( imag(Sin(:)) )
     warning( 'Input signal should be complex signal' );
end
...
%X = M * sqrt( dF .* P ) .* awgn_P1;    %user comment on Matlab share site suggested this
...
if VALIDATION_ON
     figure; 
     plot( log10(phase_noise_freq), phase_noise_power, 'o-' ); % Input SSB phase noise power
     hold on;    
     grid on;     
     plot(log10(F), 10*log10(P),'r*-'); % Input SSB phase noise power
     
     X1 = fft(phase_noise);
     plot(log10(F), 10*log10( ( (abs(X1(1:M))/max(abs(X1(1:M)))).^2 ) ./ dF(1) ), 'ks-' );% generated phase noise exp(j*x)     
     X2 = fft( 1 + j*real(x(1:N)) ); 
     plot(log10(F), 10*log10( ( (abs(X2(1:M))/max(abs(X2(1:M)))).^2 ) ./ dF(1) ), 'm>-' ); % approximation ( 1+j*x )   
     xlabel('Frequency Exponent [10^x, Hz]');
     ylabel('dBc/Hz');
     legend( ...
